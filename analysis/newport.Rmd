...
verty2 <- c(0.5, verty, nrow(mat_dp_alt)+1)

lv2 <- length(verty2)

# calculates midpoint of each section to place Model Number below x axis
verty3 <- (verty2[2:lv2] - verty2[1:(lv2 - 1)]) / 2 + verty2[1:(lv2 - 1)]
...
altnumber <- c('6 - Original', '6b - Brits')
...
Estimates of variance are consistent between models, suggesting our calculation of genetic PCs is appropriate.
...
signif_snps <- fread('output/vault/fullport/signif_snps.txt')

signif_snps$`P value` <- scientific(signif_snps$`P value`)

kable(signif_snps, 'simple', digits=2)
