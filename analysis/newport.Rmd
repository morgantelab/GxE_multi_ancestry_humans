...
Model 7 includes an additional variance component $\mathbf{i}$ that implements a modified interaction relationship matrix. The modified IRM is given by $\mathbf{K}_{top \times e} = \mathbf{K}_{top} \odot \mathbf{K}_e$ where $\mathbf{K}_{top}$ is the GRM derived from the top 10 eigenvectors of the full plink GRM.

Model 8 includes the top 10 eigenvectors of the full IRM as additional covariates (along with top 10 genetic PCs) for a total of 20 covariates in $\mathbf{X}_3$. The full IRM is the Hadamard product of the plink GRM and the full ERM, given by $\mathbf{K}_{g \times e} = \mathbf{K}_g \odot \mathbf{K}_e$.
...
## ANOVA: Environment ~ Ancestry {.tabset}

Assess environmental effects by ancestry. Analaysis of variance models where each environment was regressed across ancestry groups given by $env \sim anc$ where $env$ is a single environment and $anc$ is a factor coding for ancestry, containing the five classes:
...
The only enviornment that did not vary significantly across ancestry groups was daily activity levels (act0_d p = 0.141). All other environments had a significant difference in variance for at least one ancestry group.
