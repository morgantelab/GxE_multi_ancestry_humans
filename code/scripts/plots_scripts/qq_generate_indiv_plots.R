...
for (env in envs) {
  ...
  env_files <- list.files(results_dir, pattern=paste0("gxe_", env, "_.*\\.glm\\.linear$"), full.names=TRUE)
  ...
  for (file in env_files) {
    ...
    # Generate QQ Plot
    qqplot_file <- file.path(output_dir, paste0("qqplot_", plot_name, ".png"))
    generate_qq_plot(plink_results$P, paste("QQ Plot:", env), qqplot_file)
  }
}
