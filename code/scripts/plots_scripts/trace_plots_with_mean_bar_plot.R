...
generate_and_save_plots <- function(varabs_file_path, identifier) {
  ...
  varabs_melted2 <- varabs_melted %>% filter(Component %in% c("V_X1", "V_X2", "V_G", "V_E"))
  ...
  mean_data2 <- mean_data2 %>% filter(Component %in% c("V_X1", "V_X2", "V_G", "V_E"))
  mean_data2$Component <- factor(mean_data2$Component, levels = c("V_X1", "V_X2", "V_G", "V_E"))
