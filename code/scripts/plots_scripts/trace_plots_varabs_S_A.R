...
generate_and_save_plots <- function(varabs_file_path, identifier) {
  ...
  varabs_melted2 <- varabs_melted %>% filter(Component %in% c("V_X2", "V_E", "V_G", "V_GE"))
  
  varabs_melted2$Component <- factor(varabs_melted2$Component, levels = c("V_X2", "V_E", "V_G", "V_GE"))
  
  # Create a mean data table
  mean_data2 <- data.table(Component = names(varabs_means), Value = varabs_means)
  mean_data2 <- mean_data2 %>% filter(Component %in% c("V_X2", "V_E", "V_G", "V_GE"))
