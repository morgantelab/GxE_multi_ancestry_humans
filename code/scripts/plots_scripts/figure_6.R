...
# Create dot plots with manuscript styling and consistent y-axis
plot_list <- lapply(names(traits_titles), function(trait) {
  ggplot(summary_data_ancestries %>% filter(Trait == trait),
         aes(x = Model, y = R_squared, color = Ancestry)) +
...
ggsave("/data2/morgante_lab/ukbiobank_projects/GxE_multi_ancestry/plots/dotplot_preds_models_ancestries_combined_column.png",
       plot = final_plot,
