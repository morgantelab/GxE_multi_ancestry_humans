...
# Set the IDs in ethn_ids to NA in y
...
model <- BGLR(y=y, ETA=ETA, nIter=iter, burnIn=burnin, thin=thin, verbose=verb, saveAt=paste(opt$scratch, '/PREDs_', type, '_ethn_', ethn_number, '_X1_', sep=''))
...
write.csv(preds, file=file.path(opt$output, paste0("PREDs_", type, "_ethn_", ethn_number, "_X1.csv")), row.names=FALSE)
