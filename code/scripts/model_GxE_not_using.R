...
model <- BGLR(y=y, ETA=ETA, nIter=iter, burnIn=burnin, thin=thin, verbose=verb, saveAt=paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_', sep=''))

###Collecting results###
# Load results from BGLR output
zz0 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_mu.dat', sep=''), header=F)
colnames(zz0) <- "int"

zz1 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_ETA_G_varB.dat', sep=''), header=F)
colnames(zz1) <- "G"

zz2 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_ETA_E_varB.dat', sep=''), header=F)
colnames(zz2) <- "E"

zz9 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_varE.dat', sep=''), header=F)
colnames(zz9) <- "res"

# Combine results into a dataframe and save
VCEm <- data.frame(zz0, zz1, zz2, zz9)
write.csv(VCEm, file=file.path(opt$output, paste0("VCEm_", type, "_", grm_source, "_GE_2corr.csv")), row.names=TRUE)

# Sampled regression effects
B1 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_ETA_X1_b.dat', sep=''), header=TRUE)
B2 <- readBinMat(paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_ETA_G_b.bin', sep=''))
B3 <- readBinMat(paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_ETA_E_b.bin', sep=''))
B4 <- readBinMat(paste(opt$scratch, '/', type, '_', grm_source, '_GE_2corr_ETA_GxE_b.bin', sep=''))
...
write.csv(varabs, file=file.path(opt$output, paste0("varabs_", type, "_", grm_source, "_GE_2corr.csv")), row.names=TRUE)
