...
filtered_files <- list.files(path = plink_results_dir,
                             pattern = paste0("_", ancestry, ".", bp_type, "0s_filtered_", pval, ".txt$"),
                             full.names = TRUE)
...
for (env_term in interaction_terms) {
  ...
  print(paste("🔹 Processing environment:", env_term, "for Ancestry:", ancestry, "BP:", bp_type, "Pval:", pval))

  # Identify the corresponding filtered file for this ancestry, BP, and environment
  filtered_file <- filtered_files[grepl(paste0("gxe_", env_term, "_", ancestry, ".", bp_type, "0s_filtered_", pval, ".txt$"), filtered_files)]

  if (length(filtered_file) == 0) {
    print(paste("⚠️ No filtered SNP file found for:", env_term, "in Ancestry", ancestry, "BP:", bp_type, "Pval:", pval))
    next
  }
  ...
  if (length(snp_indices) == 0) {
    print(paste("⚠️ No SNPs found in genotype data for:", env_term, "in Ancestry", ancestry, "BP:", bp_type, "Pval:", pval))
    next
  }
...
if (num_envs_used > 0) {
  ...
  ancestry_output_file <- file.path(output_dir, paste0("Hadamard_GRM_Ancestry", ancestry, "_", bp_type, "_", pval, ".RData"))
  save(K_ancestry_grm, file = ancestry_output_file)
  print(paste("💾 Saved Hadamard GRM for Ancestry", ancestry, "BP:", bp_type, "Pval:", pval, "to", ancestry_output_file))
} else {
