...
model <- BGLR(y=y, ETA=ETA, nIter=iter, burnIn=burnin, thin=thin, verbose=verb, saveAt=paste(scratch, '/DP_X1_X2_E_run_', sep=''))
...
#
# ### intercept and fixed effects ###
# B1 <- read.table(paste(scratch, '/DP_X1_run_ETA_X_b.dat', sep=''), header=T)
#
...
B3 <- readBinMat(paste(scratch, '/SP_pcrelate_run_GE_pcrelate_pcs_plink_higher_iter_ETA_E_b.bin', sep=''))
...
#
# write.csv(varabs, file="/data2/morgante_lab/ukbiobank_projects/GxE_multi_ancestry/data/model/varabs_DP_X.csv", row.names=TRUE)
#
# print("varab saved")
#
#
