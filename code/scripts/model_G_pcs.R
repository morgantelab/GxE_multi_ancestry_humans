...
# Assuming the file name follows the pattern: "VCEm_<type>_<grm_source>.csv"
type <- parsed_info[2]  # Extract the type (e.g., sp, dp, pp)
grm_source <- gsub("\\.csv$", "", parsed_info[3])  # Extract the GRM source and remove ".csv"
...
ETA <- list(X=list(X=X, model="FIXED", saveEffects=TRUE), G=list(X=W, model="BRR", saveEffects=TRUE))
...
model <- BGLR(y=y, ETA=ETA, nIter=iter, burnIn=burnin, thin=thin, verbose=verb, saveAt=paste(opt$scratch, '/', type, '_', grm_source, '_run_', sep=''))

# Collect results
zz0 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_run_mu.dat', sep=''), header=F)
colnames(zz0) <- "int"
zz1 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_run_ETA_G_varB.dat', sep=''), header=F)
colnames(zz1) <- "G"
zz9 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_run_varE.dat', sep=''), header=F)
colnames(zz9) <- "res"

VCEm <- data.frame(zz0, zz1, zz9)
write.csv(VCEm, file=file.path(opt$output, paste0("VCEm_", type, "_", grm_source, ".csv")), row.names=TRUE)
print("VCEm results written")

# Save variance partition results
# intercept and fixed effects
B1 <- read.table(paste(opt$scratch, '/', type, '_', grm_source, '_run_ETA_X_B.dat', sep=''), header=T)

# additive genetic random effects
B2 <- readBinMat(paste(opt$scratch, '/', type, '_', grm_source, '_run_ETA_G_b.bin', sep=''))
...
write.csv(varabs, file=file.path(opt$output, paste0("varabs_", type, "_", grm_source, ".csv")), row.names=TRUE)
