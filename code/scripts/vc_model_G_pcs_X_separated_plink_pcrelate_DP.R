...
model <- BGLR(y=y_DP_pcrelate, ETA=ETA, nIter=iter, burnIn=burnin, thin=thin, verbose=verb, saveAt=paste(scratch, '/DP_pcrelate_pcs_plink_G_run_X_separated_', sep=''))
...
### intercept ###
zz0 <- read.table(paste(scratch, '/DP_pcrelate_pcs_plink_G_run_X_separated_mu.dat', sep=''), header=F); colnames(zz0) <- "int"

print("zz0 created")

### variance of ridge regression betas for the additive genetic effect ###
zz1 <- read.table(paste(scratch, '/DP_pcrelate_pcs_plink_G_run_X_separated_ETA_G_varB.dat', sep=''), header=F); colnames(zz1) <- "G"

print("zz1 created")

### variance of the residual deviations ###
zz9 <- read.table(paste(scratch, '/DP_pcrelate_pcs_plink_G_run_X_separated_varE.dat', sep=''), header=F); colnames(zz9) <- "res"
...
write.csv(VCEm_PP_pcrelate, file="/data2/morgante_lab/ukbiobank_projects/GxE_multi_ancestry/data/model/VCEm_DP_pcrelate_pcs_plink_G_run_X_separated.csv", row.names=TRUE)
...
B1 <- read.table(paste(scratch, '/DP_pcrelate_pcs_plink_G_run_X_separated_ETA_X1_b.dat', sep=''), header=T)

print("B1 done now B2")

### additive genetic random effects ###
B2 <- readBinMat(paste(scratch, '/DP_pcrelate_pcs_plink_G_run_X_separated_ETA_G_b.bin', sep=''))

### fixed effects part 2 pcs ###
#B3 <- readBinMat(paste(scratch, '/DP_pcrelate_pcs_plink_G_run_X_separated_ETA_X2_b.dat', sep=''))
B3 <- as.matrix(fread(paste(scratch, '/DP_pcrelate_pcs_plink_G_run_X_separated_ETA_X2_b.dat', sep='')))
...
write.csv(varabs, file="/data2/morgante_lab/ukbiobank_projects/GxE_multi_ancestry/data/model/varabs_DP_pcrelate_pcs_plink_G_run_X_separated.csv", row.names=TRUE)
