...
results <- data.frame(Ethnicity = character(), Trait = character(), R_squared = numeric(), Correlation = numeric(), stringsAsFactors = FALSE)
...
# Loop over ethnicities and traits
for (ethn in ethnicities) {
  for (trait in traits) {
    
    # Define file paths
    preds_file <- file.path(base_dir, paste0("PREDs_", trait, "_ethn_", ethn, "_X1_X2_G_E_GE_GEselect.csv"))
    ethn_file <- file.path(base_dir, paste0(ethn, "_ids.txt"))
    
    # Load prediction dataset
    if (!file.exists(preds_file) || !file.exists(ethn_file)) {
      cat("Skipping Ethnicity", ethn, "for trait", trait, "due to missing files\n")
      next
    }
    ...
    write_csv(preds_filled, file.path(base_dir, paste0("Updated_PREDs_", trait, "_ethn_", ethn, "_X1_X2_G_E_GE_GEselect.csv")))
    ...
    # Append results
    results <- rbind(results, data.frame(Ethnicity = ethn, Trait = trait, R_squared = r_squared, Correlation = correlation))
    
    # Print values for logging
    cat("Ethnicity:", ethn, "Trait:", trait, "R^2:", r_squared, "Correlation:", correlation, "\n")
  }
}

# Save results to CSV
write_csv(results, file.path(base_dir, "model_performance_summary_ethnicity.csv"))
