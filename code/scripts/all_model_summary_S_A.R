...
generate_filenames <- function(trait, ancestry = NULL, model, fold = NULL) {
  if (!is.null(fold)) {
...
filter_observed_na <- function(df) {
  if ("Observed" %in% colnames(df)) {
    return(df %>% filter(is.na(Observed)))
  } else {
...
update_observed_values <- function(df, filename) {
  ...
  id_match <- match(df$ID, dataset$ID)
  valid_indices <- which(!is.na(id_match) & is.na(df$Observed))
...
summary_file_path <- file.path(save_dir, "Updated_predictions_summary_S_A.csv")
write.csv(updated_summary, summary_file_path, row.names = FALSE)
message("Saved summary file: Updated_predictions_summary_S_A.csv")
