...
# Run BGLR model
 ...
 model <- BGLR(y=y_DP_pcrelate, ETA=ETA, nIter=iter, burnIn=burnin, thin=thin, verbose=verb, saveAt=paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_higher_iter_', sep=''))
...
#
# ### intercept ###
# zz0 <- read.table(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_mu.dat', sep=''), header=F); colnames(zz0) <- "int"
#
# print("zz0 created")
#
# ### variance of ridge regression betas for the additive genetic effect ###
# zz1 <- read.table(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_ETA_G_varB.dat', sep=''), header=F); colnames(zz1) <- "G"
#
# print("zz1 created")
#
# ### variance of ridge regression betas for the environmental effect ###
# zz2 <- read.table(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_ETA_E_varB.dat', sep=''), header=F); colnames(zz2) <- "E"
#
# ### variance of the residual deviations ###
# zz9 <- read.table(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_varE.dat', sep=''), header=F); colnames(zz9) <- "res"
#
# print("zz9 created")
#
...
#
# write.csv(VCEm_DP_pcrelate, file="/data2/morgante_lab/ukbiobank_projects/GxE_multi_ancestry/data/model/VCEm_DP_pcrelate_run_GE_pcrelate_pcs_plink.csv", row.names=TRUE)
# print("VCEm_DP_pcrelate written")
#
# ### Sampled regression effects ###
#
# ### intercept and fixed effects ###
# B1 <- read.table(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_ETA_X1_b.dat', sep=''), header=T)
#
# print("B1 done now B2")
#
# ### additive genetic random effects ###
# B2 <- read.table(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_ETA_X2_b.dat', sep=''), header=T)
#
# B3 <- readBinMat(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_ETA_G_b.bin', sep=''))
#
# B4 <- readBinMat(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_ETA_E_b.bin', sep=''))
#
# B5 <- readBinMat(paste(scratch, '/DP_pcrelate_run_GE_pcrelate_pcs_plink_ETA_GxE_b.bin', sep=''))
#
...
#
# write.csv(varabs, file="/data2/morgante_lab/ukbiobank_projects/GxE_multi_ancestry/data/model/varabs_DP_pcrelate_run_GE_pcrelate_pcs_plink.csv", row.names=TRUE)
#
# print("varab saved")
#
