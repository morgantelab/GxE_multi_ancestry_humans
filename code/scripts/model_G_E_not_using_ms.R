...
load(opt$envvars)
E_tcross <- tcrossprod(Emat)
E_div <- E_tcross/mean(diag(E_tcross))
...
#save
saveRDS(E_eigen, file=file.path(opt$output, paste0("E_eigen_", type, "_", grm_source, "_G_E.rds")))
...
model <- BGLR(y=y, ETA=ETA, nIter=iter, burnIn=burnin, thin=thin, verbose=verb, saveAt=paste(opt$scratch, '/', type, '_', grm_source, '_run_G_E_', sep=''))
