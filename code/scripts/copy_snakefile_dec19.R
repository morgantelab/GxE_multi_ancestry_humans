...
rule all:
  input:
  expand(config["model_dir"] + "/" + "PREDs_{bp}_ethn_{ancestry}_X1.csv", bp=config["BP"], ancestry=config["Ancestry"]), expand(config["model_dir"] + "/" + "PREDs_{bp}_ethn_{ancestry}_X1_X2.csv", bp=config["BP"], ancestry=config["Ancestry"]), expand(config["model_dir"] + "/" + "PREDs_{bp}_ethn_{ancestry}_X1_X2_G.csv", bp=config["BP"], ancestry=config["Ancestry"]), expand(config["model_dir"] + "/" + "PREDs_{bp}_ethn_{ancestry}_X1_X2_E.csv", bp=config["BP"], ancestry=config["Ancestry"]), expand(config["model_dir"] + "/" + "PREDs_{bp}_ethn_{ancestry}_X1_X2_G_E.csv", bp=config["BP"], ancestry=config["Ancestry"]), expand(config["model_dir"] + "/" + "PREDs_{bp}_ethn_{ancestry}_X1_X2_G_E_GE.csv", bp=config["BP"], ancestry=config["Ancestry"])
...
# # Step 9: pca for grm by plink/gcta

#rule pca_for_grm_by_plink:
#    input:
#        grm_by_plink=config["filtered_chr_dir"] + "/" + "grm_by_plink.grm.bin",
#    output:
#        pca_gcta=config["filtered_chr_dir"] + "/" + "pca_by_gcta.eigenval"
#    params:
#        gcta_executable = config["gcta_executable"],
#        THREADS=50,
#        IPREFIX=config["filtered_chr_dir"] + "/" + "grm_by_plink",
#        OPREFIX="pca_by_gcta",
#        CHRDIR=config["filtered_chr_dir"]
#    resources:
#        cpus=10, mem_mb=15000, time_min=3000
#    shell:
#        """
#        cd {params.CHRDIR}
#
#        {params.gcta_executable} \
#        --grm {params.IPREFIX} \
...
# Step 10: pca of grm by plink using eigen in R instead of gcta pca
