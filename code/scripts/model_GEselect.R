...
#
# ### intercept ###
# zz0 <- read.table(paste(scratch, '/DP_run_GEselect_mu.dat', sep=''), header=F); colnames(zz0) <- "int"
#
# print("zz0 created")
#
# ### variance of ridge regression betas for the additive genetic effect ###
# zz1 <- read.table(paste(scratch, '/DP_run_GEselect_ETA_G_varB.dat', sep=''), header=F); colnames(zz1) <- "G"
#
# print("zz1 created")
#
# ### variance of ridge regression betas for the environmental effect ###
# zz2 <- read.table(paste(scratch, '/DP_run_GEselect_ETA_E_varB.dat', sep=''), header=F); colnames(zz2) <- "E"
#
# ### variance of the residual deviations ###
# zz9 <- read.table(paste(scratch, '/DP_run_GEselect_varE.dat', sep=''), header=F); colnames(zz9) <- "res"
#
# print("zz9 created")
#
...
#
# write.csv(VCEm_DP_pcrelate, file="/data2/morgante_lab/ukbiobank_projects/GxE_multi_ancestry/data/model/VCEm_DP_run_GEselect.csv", row.names=TRUE)
# print("VCEm_DP_pcrelate written")
#
# ### Sampled regression effects ###
#
# ### intercept and fixed effects ###
# B1 <- read.table(paste(scratch, '/DP_run_GEselect_ETA_X1_b.dat', sep=''), header=T)
#
# print("B1 done now B2")
#
# ### additive genetic random effects ###
# B2 <- read.table(paste(scratch, '/DP_run_GEselect_ETA_X2_b.dat', sep=''), header=T)
#
# B3 <- readBinMat(paste(scratch, '/DP_run_GEselect_ETA_G_b.bin', sep=''))
#
# B4 <- readBinMat(paste(scratch, '/DP_run_GEselect_ETA_E_b.bin', sep=''))
#
# B5 <- readBinMat(paste(scratch, '/DP_run_GEselect_ETA_GxE_b.bin', sep=''))

# B6 <- readBinMat(paste(scratch, '/DP_run_GEselect_ETA_GxEselect_b.bin', sep=''))
#
# ### dataframe with variance partition ###
# varabs <- matrix(NA, nrow_varabs, 6); colnames(varabs) <- c("V_X1", "V_X2", "V_G", "V_E", "V_GxE", "V_GxEselect")
#
...
# varabs[, 6] <- matrixStats::colVars(tcrossprod(ETA$GxEselect$X, B6))
# print("varab cols done now saving varab")
#
# write.csv(varabs, file="/data2/morgante_lab/ukbiobank_projects/GxE_multi_ancestry/data/model/varabs_DP_run_GEselect.csv", row.names=TRUE)
#
# print("varab saved")
#
